...
void mainSequence(RenderWindow& window) {
	...
	sf::Vector2f starshipInitialPosition = sf::Vector2f(WINDOW_WIDTH / 2, WINDOW_HEIGHT - BOTTOM_BAR_HEIGHT);
	Starship starship(starshipInitialPosition);

	sf::Vector2f spiderInitialPosition = sf::Vector2f(WINDOW_WIDTH, 0.5 * WINDOW_HEIGHT);
	Spider spider(spiderInitialPosition);

	Centipedes centipedes(NUM_CENTIPEDE_SEGMENTS, sf::Vector2f((int)(NUM_HORIZONTAL_GRID / 2), 0));

	LivesDisplay livesDisplay(NUM_LIVES, sf::Vector2f(LIVES_DISPLAY_POSITION_X, LIVES_DISPLAY_POSITION_Y));
	...
	while (window.isOpen())
	{
		...
		starship.refresh();
		spider.refresh();
		auto [scoreChange, newMushroomPosList] = centipedes.refresh(mushrooms, starship);
		...
		starship.removeDestroyedBlasts();
		mushrooms.remove_if(
			[](auto& mushroom) { return mushroom.getState() == Mushroom::MushroomState::DESTROYED;  }
		);
		...
		scoreDisplay.setPosition(sf::Vector2f(WINDOW_WIDTH / 2 - scoreDisplay.getGlobalBounds().getSize().x / 2.0, SCORE_Y_OFFSET));
...
void gameOverSequence(RenderWindow& window, bool won)
{
	...
	textDisplay.setPosition(sf::Vector2f(WINDOW_WIDTH / 2 - size.x / 2.0, WINDOW_HEIGHT / 2 - size.y / 2.0));
...
std::list<Mushroom> populateMushrooms(std::list<Mushroom>& mushrooms){
	...
	std::uniform_int_distribution<int> uniformDistributionHorizontal(0, NUM_HORIZONTAL_GRID - 1);
	std::uniform_int_distribution<int> uniformDistributionVertical(0, NUM_VERTICAL_GRID - 1);
	...
	for (auto i = currentMushroomCount; i < NUM_MUSHROOMS; i++) {
		...
		do {
			...
			mushroomCoordinate = sf::Vector2f(
				...
				uniformDistributionVertical(gen) * float(GRID_SIZE_PIXELS) + TOP_BAR_HEIGHT
			);
